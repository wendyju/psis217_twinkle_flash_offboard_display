...
// WGJ added shuffling of leds, to help twinkle effects, and a bang when the threshold is crossed.
// Also, ability to drive offboard neopixels!
//
...
#define PIN            A1
#define NUMPIXELS      60 //this works for 2 m of strand! CRAZY GREAT!!!
...
void setup() {
  ...
  // uncomment this to make the display work!
  
//  if (display.begin(SSD1306_SWITCHCAPVCC, 0x3C)) { // initialize with the I2C addr 0x3C (for the 128x32)
//    hasDisplay = true;
...
void loop() {
  ...
  float maxSound = -1.0;
  for (int i = 0; i < 10; i++) {
    maxSound = max(maxSound, p);
  }

  if (maxSound > threshold) {
    for (int i = 0; i < numLEDs; i++) {
      CircuitPlayground.strip.setPixelColor(i, 200, 200, 200);
    ...
    p           = map(p, 60, 140, 0, 350); // Scale to 0-350 (may overflow) . <-previously 56 turned to 60
    ...
    for (i = 0; i < numLEDs; i++) {         // For each NeoPixel...
    ...
    // this section here shuffles the array of pixels so that it twinkles instead of making a bar

    for (int i = 0; i < numLEDs; i++) {
      int n = random(0, numLEDs);  // Integer from 0 to questionCount-1
      temp = CircuitPlayground.strip.getPixelColor(n);
      CircuitPlayground.strip.setPixelColor(n, CircuitPlayground.strip.getPixelColor(i));
      CircuitPlayground.strip.setPixelColor(i, temp);
    }
    for (int i = 0; i < NUMPIXELS; i++) {
      temp = CircuitPlayground.strip.getPixelColor(i % CircuitPlayground.strip.numPixels());
...
void displayReading(int16_t newReading) {

  display.setTextSize(1);
  display.setTextColor(WHITE);
  display.setCursor(0, 0);
  display.println("Sound Reading");
  display.setCursor(25, 12);
  display.setTextColor(BLACK, WHITE); // 'inverted' text
  display.setTextSize(2);
  display.setTextColor(WHITE);
  display.println(newReading);
  display.display();
  // delay(50);
  display.clearDisplay();
}
